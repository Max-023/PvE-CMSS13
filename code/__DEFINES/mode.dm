...
/// Once the round is starting, we want to reference the ticker. Otherwise the cache.
#define GET_CURRENT_MODE (SSticker.current_state >= GAME_STATE_SETTING_UP ? SSticker.mode : config.mode_cache)
/// Could also reference RoleAuthority, but this is meant for the actual mode roles, in case role mapping is revived (I hope not).
#define GET_CURRENT_MODE_ROLES (SSticker.current_state >= GAME_STATE_SETTING_UP ? SSticker.mode.get_roles_list() : config.mode_cache.get_roles_list())
...
#define BLURB_USCM_COMBAT JOB_CO, JOB_XO, JOB_SO, JOB_WO_CO, JOB_WO_XO, JOB_WO_CHIEF_POLICE, JOB_WO_SO, JOB_WO_CREWMAN, JOB_WO_POLICE, JOB_SEA,\
						...
						JOB_USCM_GROUND_CO, JOB_USCM_GROUND_AO, JOB_USCM_GROUND_SQUAD_LEADER, JOB_USCM_GROUND_SQUAD_TEAM_LEADER, JOB_USCM_GROUND_SQUAD_MEDIC, JOB_USCM_GROUND_SQUAD_SPECIALIST, JOB_USCM_GROUND_SQUAD_SMARTGUNNER, JOB_USCM_GROUND_SQUAD_MARINE
...
#define FACTION_USCM_GROUND "USCM Outpost"
