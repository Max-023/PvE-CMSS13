//A platoon of Vietnam era Marines, to replace the aged Dutch's Dozen.

/datum/equipment_preset/nam
	name = JOB_NAM_RIFLEMAN
	paygrades = list(PAY_SHORT_PVT = JOB_PLAYTIME_TIER_0)
	assignment = JOB_NAM_RIFLEMAN
	flags = EQUIPMENT_PRESET_EXTRA
	faction = FACTION_USMC

	skills = /datum/skills/nam

/datum/equipment_preset/dutch/New()
	..()
	rank = assignment

/datum/equipment_preset/nam/load_name(mob/living/carbon/human/new_human, randomise)
...
/datum/equipment_preset/nam/load_gear(mob/living/carbon/human/new_human)


	new_human.equip_to_slot_or_del(new /obj/item/clothing/head/helmet/marine/nam(new_human), WEAR_HEAD)
	...
	new_human.equip_to_slot_or_del(new /obj/item/clothing/under/marine/veteran/nam(new_human), WEAR_BODY)
